...
- В шапку приложения встроена кнопка **«Отчёт PDF»**, которая **мгновенно открывает официальный научно-технический отчёт в формате PDF в новой вкладке браузера** (`target="_blank"`).
...
- Приложение и статический PDF-отчёт автоматически деплоятся на GitHub Pages.
...
├── backend/
│   ├── app.py                    # FastAPI микросервис симуляции, экспорта и отчёта
│   └── requirements.txt          # Зависимости Python (FastAPI, NumPy, NetworkX, ReportLab)
├── frontend/
│   ├── public/
│   │   └── report.pdf            # Научно-технический отчёт в PDF
│   ├── src/
│   │   ├── components/
